            ...
            api.sessionexists(id, function (value) {
                ...
                api.editsession(id, httpproxy, enableShuffling, function () {
                    ...
                    api.shuffleDict(id, function (shuffleDict) {
                        if (!shuffleDict) {
                            document.getElementById('proxy-frame').src = '/' + id + '/' + url;
                        } else {
                            var shuffler = new StrShuffler(shuffleDict);
                            document.getElementById('proxy-frame').src = '/' + id + '/' + shuffler.shuffle(url);
                        }
                    });
                });
            });
...
document.getElementById('session-refresh').onclick = function () {
    var iframe = document.getElementById('proxy-frame');
    if (!iframe.src) return setError('No page loaded to refresh');
    ...
    // Force links in iframe to open in the same iframe (works for same-origin pages)
...
iframe.onload = function () {
    try {
        ...
        links.forEach(link => link.target = "_self"); // force links to open in iframe
    } catch (e) {
        console.warn('Cannot access cross-origin iframe links', e);
    }
};
